...
Weather and TTC daily data are merged by date (`inner join`). This means the final time window is the overlap of both sources. Although weather is requested through 2026-03-31, TTC daily records currently extend through 2026-01-31, so the merged table ends at 2026-01-31. The final analysis table contains `{python} n_days` days from `{python} start_date` to `{python} end_date`.
