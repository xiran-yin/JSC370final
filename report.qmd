---
title: "How Do Weather Conditions Affect Daily TTC Subway Delays in Toronto?"
author: "Xiran Yin"
format:
  html:
    toc: true
...
p_precip_label = "<0.001" if p_precip < 0.001 else f"{p_precip:.4f}"
coef_snow = float(tbl_hyp_terms.loc[tbl_hyp_terms["term"] == "snowfall_cm", "coef"].iloc[0])
p_snow = float(tbl_hyp_terms.loc[tbl_hyp_terms["term"] == "snowfall_cm", "p_value"].iloc[0])
p_snow_label = "<0.001" if p_snow < 0.001 else f"{p_snow:.4f}"
...
The merged panel spans `{python} start_date` to `{python} end_date` (calendar days: `{python} n_days`). In the current TTC extract, incident records are present on `{python} n_days_with_incidents` of `{python} n_days` calendar days, so the modeling sample and the full date panel coincide.
...
Here, `H0_i` denotes the null hypothesis for research question `i`, and `H1_i` denotes the corresponding alternative hypothesis.
...
The final daily table contains `{python} n_days` calendar days from `{python} start_date` to `{python} end_date`, of which `{python} n_days_with_incidents` have at least one recorded incident and `{python} n_zero_incident_days` have zero recorded incidents. In this run, annual coverage is complete (`{python} round(min_cov, 1)`% minimum by year), so the incident-recorded subset (`{python} n_model_days` days) equals the full panel.
All regression, classification, and hypothesis-testing results are still estimated on incident-recorded days by design; with current coverage, this is equivalent to the full date range.
...
Exploratory diagnostics were done with Pandas summary tables (coverage/range checks), Seaborn/Matplotlib static plots (distribution and seasonal structure), and [Plotly interactive visuals](viz.html) (time-series and weather/day-type comparisons).
The exploratory workflow was refined from the midterm implementation documented in [midterm_final.html](https://xiran-yin.github.io/JSC370/midterm_final.html), then updated for the final project’s expanded date range and model comparisons.
...
As a robustness check, five-fold cross-validation was run on incident-recorded days: RF regression reached CV RMSE `{python} round(cv_reg_rmse, 2)` (CV R² `{python} round(cv_reg_r2, 3)`), while classification CV ROC-AUC was `{python} round(cv_rf_auc, 3)` for Random Forest and `{python} round(cv_logit_auc, 3)` for Logistic Regression.
...
#| fig-cap: "Figure 1. Monthly trend/precipitation (top) and incident-day weather-category delay distribution (bottom)."
...
#| tbl-cap: "Weather-category descriptive summary on incident-recorded days."
tbl_weather.round(2)
```

```{python}
#| label: tbl-ols
#| tbl-cap: "OLS coefficient summary for daily total delay minutes (incident-recorded days)."
ols_df = daily.loc[
    daily["total_incidents"] > 0,
...
ols_table = pd.DataFrame(
    {
        ...
        "t": ols_fit.tvalues.values,
        "p_value": ols_fit.pvalues.values,
        "ci_95_low": ols_fit.conf_int()[0].values,
        "ci_95_high": ols_fit.conf_int()[1].values,
    }
)
ols_table = ols_table[ols_table["term"] != "Intercept"].copy()
ols_table["p_value"] = ols_table["p_value"].apply(lambda p: "<0.001" if p < 0.001 else f"{p:.4f}")
ols_table.round(3)
```

Taken together, the two tables provide a descriptive-to-model bridge for Figure 1. The weather summary table reports raw group-level patterns (including the heavy-weather mean of `{python} round(key["heavy_mean_delay"], 1)` minutes versus `{python} round(key["clear_mean_delay"], 1)` on clear days). The OLS model is estimated on incident-recorded days with complete covariates (n = `{python} len(ols_df)`) using `total_delay_mins ~ precip_mm + snowfall_cm + mean_temp_c + is_weekend_int`; it suggests positive adjusted associations for both precipitation (beta = `{python} round(coef_precip, 3)`, p `{python} p_precip_label`) and snowfall (beta = `{python} round(coef_snow, 3)`, p `{python} p_snow_label`), while weekend days remain lower than weekdays (beta = `{python} round(coef_weekend, 3)`).
...
Table 1 shows that `{python} best_reg_model` has the lowest held-out RMSE (`{python} round(best_reg_rmse, 2)`) with R² `{python} round(best_reg_r2, 3)`. The gain over linear regression is modest (`{python} round(rmse_gain_pct, 2)`% RMSE reduction), which indicates that weather variables improve fit but explain only part of day-level delay variation. Table 2 shows moderate severe-day discrimination: `{python} best_cls_model` has the top ROC-AUC (`{python} round(best_cls_auc, 3)`), which is above random (0.5) but still limited. In practical terms, this means severe days are ranked above non-severe days only moderately well. Logistic Regression also has the strongest recall/F1 in this comparison (recall `{python} round(float(tbl_cls.loc[tbl_cls["model"]=="Logistic Regression","recall"].iloc[0]), 3)`, F1 `{python} round(float(tbl_cls.loc[tbl_cls["model"]=="Logistic Regression","f1"].iloc[0]), 3)`), so it captures more severe days at the cost of more false alarms and lower overall accuracy than some tree models. This pattern is consistent with the OLS findings: weather effects are statistically detectable, but prediction remains limited without additional operational features. Feature-importance outputs are reported in the interactive-visualization artifacts and emphasize precipitation, wind, and temperature predictors.
...
RQ1: reject H0_1; precipitation is positively associated with delay (beta = `{python} round(coef_precip, 3)`, p = `{python} p_precip_label`) and heavy-weather mean delay is `{python} round(heavy_clear_ratio, 2)`x clear-weather delay.  
RQ2: reject H0_2; weekend coefficient is negative and significant (beta = `{python} round(coef_weekend, 3)`, p = `{python} "<0.001" if p_weekend < 0.001 else f"{p_weekend:.4f}"`), indicating higher weekday expected delay.  
RQ3: reject H0_3 with modest gain; best RMSE improves by `{python} round(rmse_gain_pct, 2)`% over linear baseline.  
RQ4: reject H0_4 with moderate effect; best ROC-AUC is `{python} round(best_cls_auc, 2)`, above random ranking (0.5).
...
4. The model uses weather and calendar predictors only; unobserved operational factors (e.g., ridership demand, planned service changes, line disruptions) may explain additional variation.
5. Findings are associational/predictive, not causal.
