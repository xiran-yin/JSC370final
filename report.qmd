...
Figure 1 is a two-panel diagnostic: the top panel is the monthly trend view and the bottom panel is the weather-category distribution view. The top panel now shows continuous coverage across the full 2014-2026 window, with visibly higher monthly totals in recent years. The bottom panel shows a higher center and broader upper tail under heavy weather.

```{python}
fig1_stats = pd.DataFrame(
    {
        "Statistic": [
            "Heavy-weather mean delay (min)",
            "Clear-weather mean delay (min)",
            "Heavy/Clear delay ratio",
            "Precipitation slope beta (OLS)",
            "Precipitation p-value (OLS)",
        ],
        "Value": [
            round(key["heavy_mean_delay"], 1),
            round(key["clear_mean_delay"], 1),
            f'{round(heavy_clear_ratio, 2)}x',
            round(coef_precip, 3),
            p_precip_label,
        ],
        "Source": [
            "outputs/table_weather_summary.csv",
            "outputs/table_weather_summary.csv",
            "outputs/key_numbers.json",
            "outputs/table_hypothesis_terms.csv",
            "outputs/table_hypothesis_terms.csv",
        ],
    }
)
...
The table above shows where the Figure 1 headline numbers come from; all of them are generated by `scripts/final_pipeline.py` from `data/processed_daily.csv`.
