...
Figure 1 is a two-panel diagnostic: the left panel is the monthly trend view and the right panel is the weather-category distribution view. The monthly panel now shows continuous coverage across the full 2014-2026 window, with visibly higher monthly totals in recent years. The right panel shows a higher center and broader upper tail under heavy weather. Summary statistics are consistent with the plot patterns: heavy weather days average `{python} round(key["heavy_mean_delay"], 1)` minutes versus `{python} round(key["clear_mean_delay"], 1)` minutes on clear days (ratio `{python} round(heavy_clear_ratio, 2)`x), with positive precipitation slope (beta = `{python} round(coef_precip, 3)`, p = `{python} p_precip_label`). These values are generated by `scripts/final_pipeline.py` from `data/processed_daily.csv`, with weather-group means written to `outputs/table_weather_summary.csv` and regression coefficient terms written to `outputs/table_hypothesis_terms.csv`.
...
1. **RQ1:** reject H0_1; precipitation is positively associated with delay (beta = `{python} round(coef_precip, 3)`, p = `{python} p_precip_label`) and heavy-weather mean delay is `{python} round(heavy_clear_ratio, 2)`x clear-weather delay.
