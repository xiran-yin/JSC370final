...
reg_rank = tbl_reg.sort_values("rmse").reset_index(drop=True)
cls_rank = tbl_cls.sort_values("roc_auc", ascending=False).reset_index(drop=True)
reg_order_text = ", ".join([f'{row["model"]} {row["rmse"]:.2f}' for _, row in reg_rank.iterrows()])
cls_order_text = ", ".join([f'{row["model"]} {row["roc_auc"]:.3f}' for _, row in cls_rank.iterrows()])
...
Exploratory diagnostics were done with Pandas summary tables (coverage/range checks), Seaborn/Matplotlib static plots (distribution and seasonal structure), and [Plotly interactive visuals](https://xiran-yin.github.io/JSC370final/viz.html) (time-series and weather/day-type comparisons).
...
fig, axes = plt.subplots(2, 1, figsize=(9.5, 5.8))
...
ols_table_display = ols_table[["term", "coef", "p_value", "ci_95_low", "ci_95_high"]].rename(
    columns={
        "term": "term",
        "coef": "coef",
        "p_value": "p",
...
ols_table_display.round(3)
```

Figure 1 and the OLS table provide a concise descriptive-to-model bridge. Heavy-weather days average `{python} round(key["heavy_mean_delay"], 1)` minutes versus `{python} round(key["clear_mean_delay"], 1)` on clear days (ratio `{python} round(heavy_clear_ratio, 2)`x). The OLS model, estimated on incident-recorded days (n = `{python} len(ols_df)`), shows positive adjusted associations for precipitation (beta = `{python} round(coef_precip, 3)`, p `{python} p_precip_label`) and snowfall (beta = `{python} round(coef_snow, 3)`, p `{python} p_snow_label`), with lower expected delay on weekends (beta = `{python} round(coef_weekend, 3)`).
...
tbl_reg_display = tbl_reg.copy()
tbl_reg_display["model"] = tbl_reg_display["model"].replace(
    {
...
tbl_reg_display.round(4)
```

```{python}
#| label: tbl-cls
#| tbl-cap: "Classification model comparison for severe-delay-day detection on the held-out test set."
tbl_cls_display = tbl_cls.copy()
tbl_cls_display["model"] = tbl_cls_display["model"].replace(
    {
...
tbl_cls_display.round(4)
```

Table 1 shows that `{python} best_reg_model` has the lowest held-out RMSE (`{python} round(best_reg_rmse, 2)`) with R² `{python} round(best_reg_r2, 3)`. Ranked from best to worst RMSE, the regression models are: `{python} reg_order_text`; the gain over linear regression is modest (`{python} round(rmse_gain_pct, 2)`% RMSE reduction), indicating limited explained variation at day level. Table 2 shows moderate severe-day discrimination: `{python} best_cls_model` has the top ROC-AUC (`{python} round(best_cls_auc, 3)`), and classification models rank as `{python} cls_order_text`. Logistic Regression also has the strongest recall/F1 in this comparison (recall `{python} round(float(tbl_cls.loc[tbl_cls["model"]=="Logistic Regression","recall"].iloc[0]), 3)`, F1 `{python} round(float(tbl_cls.loc[tbl_cls["model"]=="Logistic Regression","f1"].iloc[0]), 3)`), so it captures more severe days at the cost of more false alarms. Feature-importance outputs are reported in the interactive-visualization artifacts and emphasize precipitation, wind, and temperature predictors; an interactive ROC comparison is provided in [Interactive Visualizations](https://xiran-yin.github.io/JSC370final/viz.html) Figure 4.
